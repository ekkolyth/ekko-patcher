...
/*
* Rom Patcher JS core
* A ROM patcher/builder made in JavaScript
* By Marc Robledo https://www.marcrobledo.com
* License: MIT
*/

import { BinFile } from './BinFile';
import { IPS, UPS, BPS, APS, APSGBA, RUP, PPF, BDF, PMSR, VCDIFF } from './formats';
...
const RomPatcher = {
	...
	createPatch: function (originalFile, modifiedFile, format, metadata) {
		...
		if (
			!(format === 'ppf' && originalFile.fileSize > modifiedFile.fileSize) &&
			modifiedFile.hashCRC32() !== patch.apply(originalFile).hashCRC32()
		) {
			//throw new Error('Unexpected error: verification failed.');
		}
		return patch;
	},
